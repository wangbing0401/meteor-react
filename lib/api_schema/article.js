...
ArticleSchema = new SimpleSchema({
    ...
    type:{
        type: Number,
        label:'文章类型1.H5 2.angular 3.react 4.meteor 0.其他',
        optional: true,
        defaultValue: 0
    }
});
...
if(Meteor.isServer){
    Meteor.publish('article_count', function(){
        Counts.publish(this, 'article_count', Article.find());
    });
    Meteor.publish("get_article_list", function(mark, article_type){
        console.log(article_type);
        var result = Article.find({type:article_type}, {limit:mark*10});
        return result;
    });
